...
def process_markdown_files(readme_path, archive_path, failed_urls, link_history, dry_run=False):
    """
    Process the README.md file, moving items with persistently failed links to archived.md.
    Returns a list of URLs that were archived.
    """
    # Identify links that have failed 3 or more times
    urls_to_archive = {url for url, count in link_history.items() 
                      if count >= 3 and url in failed_urls}
        ...
        if is_list_start:
            ...
            if in_list_item and current_item_lines:
                ...
                for url in urls_to_archive:
                    ...
                    if re.search(rf'(\[.*?\]\({re.escape(url)}\))|{re.escape(url)}', item_content):
                        should_archive = True
                        found_url = url
                        break
    ...
    if dry_run:
        ...
        print(f"[DRY RUN] {len(archived_urls)} items would be moved from {readme_path} to {archive_path}")
    ...
    if not archive.exists() or archive.stat().st_size == 0:
        ...
        mode = 'w'
        header = f"# Archived Entries\n\nEntries moved here due to persistently failing links detected by the automated link checker.\n\n## Archived on {timestamp}\n\n"
...
def main():
    parser = argparse.ArgumentParser(description='Process link checker reports and handle dead links')
    ...
    parser.add_argument('--dry-run', action='store_true', help='Run in dry-run mode (no changes written)')
    parser.add_argument('--force-count', type=int, help='Force failure count for specified URLs (for testing)')
    parser.add_argument('--force-urls', nargs='+', help='URLs to force the failure count on (for testing)')
    ...
    if args.force_count and args.force_urls:
        for url in args.force_urls:
            ...
            failed_urls.add(url)
        print(f"Forced failure count {args.force_count} for {len(args.force_urls)} URLs")
    else:
    ...
    archived_urls = process_markdown_files(args.readme, args.archive, failed_urls, link_history, args.dry_run)
